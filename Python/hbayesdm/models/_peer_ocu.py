...
from hbayesdm.preprocess_funcs import peer_preprocess_func

__all__ = ['peer_ocu']


class PeerOcu(TaskModel):
    ...
    _preprocess_func = peer_preprocess_func
...
def peer_ocu(
    ...
    Hierarchical Bayesian Modeling of the Peer Influence Task [Chung2015]_
    using Other-Conferred Utility (OCU) Model  with the following parameters:
    "rho" (risk preference), "tau" (inverse temperature), "ocu" (other-conferred utility).

    

    .. [Chung2015] Chung, D., Christopoulos, G. I., King-Casas, B., Ball, S. B., & Chiu, P. H. (2015). Social signals of safety and risk confer utility and have asymmetric effects on observers' choices. Nature Neuroscience, 18(6), 912-916.
    ...
    **tab-delimited** text file, whose rows represent trial-by-trial observations
    ...
    For the Peer Influence Task, there should be 8 columns of data
    with the labels "subjID", "condition", "p_gamble", "safe_Hpayoff", "safe_Lpayoff", "risky_Hpayoff", "risky_Lpayoff", "choice". It is not necessary for the columns to be
