...
__all__ = ['pstRT_rlddm1']


class PstrtRlddm1(TaskModel):
...
def pstRT_rlddm1(
    ...
    using Reinforcement Learning Drift Diffusion Model 1 [Pedersen2017]_ with the following parameters:
    "a" (boundary separation), "tau" (non-decision time), "v" (drift rate scaling), "alpha" (learning rate).
    ...
    **tab-delimited** text file, whose rows represent trial-by-trial observations
    ...
    For the Probabilistic Selection Task (with RT data), there should be 6 columns of data
    with the labels "subjID", "cond", "prob", "choice", "RT", "feedback". It is not necessary for the columns to be
    ...
    data
        ...
        Data columns should be labeled as: "subjID", "cond", "prob", "choice", "RT", "feedback".
    niter
