...
from hbayesdm.preprocess_funcs import rdt_preprocess_func

__all__ = ['rdt_happiness']


class RdtHappiness(TaskModel):
    ...
    _preprocess_func = rdt_preprocess_func
...
def rdt_happiness(
    ...
    Hierarchical Bayesian Modeling of the Risky Decision Task 
    using Happiness Computational Model [Rutledge2014]_ with the following parameters:
    "w0" (baseline), "w1" (weight of certain rewards), "w2" (weight of expected values), "w3" (weight of reward prediction errors), "gam" (forgetting factor), "sig" (standard deviation of error).

    

    
    .. [Rutledge2014] Rutledge, R. B., Skandali, N., Dayan, P., & Dolan, R. J. (2014). A computational and neural model of momentary subjective well-being. Proceedings of the National Academy of Sciences, 111(33), 12252-12257.
    ...
    **tab-delimited** text file, whose rows represent trial-by-trial observations
    ...
    For the Risky Decision Task, there should be 9 columns of data
    with the labels "subjID", "gain", "loss", "cert", "type", "gamble", "outcome", "happy", "RT_happy". It is not necessary for the columns to be
