...
def message_model_regressor_parameter(regressors):
    if regressors:
        return "For this model they are: " + format_list(regressors, fmt='"{}"', sep=", ")
    else:
...
def main(info_fn):
    ...
    if info["task_name"].get("cite"):
        task_cite = OrderedDict((shortify(cite), cite) for cite in info["task_name"]["cite"])
    else:
        task_cite = {}

    if info["model_name"].get("cite"):
        model_cite = OrderedDict((shortify(cite), cite) for cite in info["model_name"]["cite"])
    else:
        model_cite = {}
    ...
    with open(TEMPLATE_DOCS, "r") as f:
        docstring_template = f.read().format(
            ...
            task_cite_short=format_list(task_cite, fmt="[{}]_", sep=", "),
            task_cite_long=format_dict(task_cite, fmt=".. [{}] {}", sep="\n    "),
            model_name=model_name_desc if model_name_desc is not None else "",
            model_cite_short=format_list(model_cite, fmt="[{}]_", sep=", "),
            model_cite_long=format_dict(
                OrderedDict((k, v) for k, v in model_cite.items() if k not in task_cite), fmt=".. [{}] {}", sep="\n    "
            ),
            model_type=model_type_desc if model_type_desc is not None else "",
            notes=format_list(
                info.get("notes") if info.get("notes") else [], fmt=".. note::\n        {}", sep="\n\n    "
            ),
            ...
            data_columns=format_list(info["data_columns"], fmt='"{}"', sep=", "),
            data_columns_len=len(info["data_columns"]),
            data_columns_details=format_dict(info["data_columns"], fmt='- "{}": {}', sep="\n    "),
            parameters=format_dict(info["parameters"], fmt='"{}" ({})', sep=", ", pre=lambda v: v["desc"]),
            model_regressor_parameter=message_model_regressor_parameter(info["regressors"]),
            model_regressor_return=message_model_regressor_return(info["regressors"]),
            postpreds=message_postpreds(info["postpreds"]),
            additional_args=message_additional_args(info["additional_args"]),
        )
    ...
    with open(TEMPLATE_CODE, "r") as f:
        code_template = f.read().format(
            ...
            parameters=format_dict(
                ...
                fmt="('{}', ({})),",
                sep="\n                ",
                pre=lambda v: ", ".join(str(x).replace("Inf", "inf") for x in v["info"]),
            ),
            regressors=format_dict(
                info.get("regressors") if info.get("regressors") else {}, fmt="('{}', {}),", sep="\n                "
            ),
            postpreds=format_list(info.get("postpreds") if info.get("postpreds") else [], fmt="'{}'", sep=", "),
            parameters_desc=format_dict(
                info["parameters"], fmt="('{}', '{}'),", sep="\n                ", pre=lambda v: v["desc"]
            ),
...
if __name__ == "__main__":
    ...
    parser.add_argument("-v", "--verbose", help="Whether to print its process.", action="store_true")
    parser.add_argument("info_files", help="YAML-formatted file(s) for model information.", type=str, nargs="*")
    ...
    for i, info_fn in enumerate(info_fns):
        main(info_fn)
        if args.verbose:
            print("[{:2d} / {:2d}] Done for {}".format(i + 1, num_models, info_fn))
