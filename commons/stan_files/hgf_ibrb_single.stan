...
data {
  ...
  array[T] int<lower=-1, upper=1> u; // inputs
  array[T] int<lower=-1, upper=1> y; // responses (choices that subject made)
...
transformed parameters {
  ...
  if (n_free_kappa > 0) {
    for (i in 1 : n_free_kappa) {
      ...
      kappa[l] = inv_logit_with_bounds(logit_kappa[i], kappa_lower[l],
                                       kappa_upper[l]);
    }
  }
  if (n_fixed_kappa > 0) {
  ...
  if (n_free_omega > 0) {
    for (i in 1 : n_free_omega) {
      ...
      omega[l] = inv_logit_with_bounds(logit_omega[i], omega_lower[l],
                                       omega_upper[l]);
    }
  }
  if (n_fixed_omega > 0) {
