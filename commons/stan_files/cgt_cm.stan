...
model {
  ...
  for (i in 1 : N) {
    ...
    // Model
    for (t in 1 : Tsubj[i]) {
      // Need to set trial/vectorized parts correctly
      // Assign probability of choosing color (fix bias red)
      col_util[1] = (c[i] * pow(prop_red[i, t], alpha[i]))
                    / (c[i] * pow(prop_red[i, t], alpha[i])
                       + ((1 - c[i]) * pow(1 - prop_red[i, t], alpha[i])));
      ...
      // Utility of all bets
      bet_util = gain_util * prop_chosen[i, t]
                 + loss_util * (1 - prop_chosen[i, t]);
  ...
  
  {
    ...
    for (i in 1 : N) {
      ...
      for (t in 1 : Tsubj[i]) {
        ...
        // Increment log likelihood for color choice
        log_lik[i] = log_lik[i]
                     + categorical_lpmf(col_chosen[i, t] | col_util);
        ...
        // Increment log likelihood for choosing bet
        log_lik[i] += categorical_logit_lpmf(bet_chosen[i, t] | bet_util
                                                                * gamma[i]);
        // Posterior prediction for bet
