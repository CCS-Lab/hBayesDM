...
model {
  ...
  for (i in 1 : N) {
    ...
    for (t in 1 : Tsubj[i]) {
      ...
      sig2_new = (nu_old / nu_new) * sig2_old
                 + (1 / nu_new) * (k_old / k_new)
                   * pow(offer[i, t] - mu_old, 2);
      
      PE = offer[i, t] - mu_old;
      util = offer[i, t] - alpha[i] * fmax(mu_new - offer[i, t], 0.0)
             - beta[i] * fmax(offer[i, t] - mu_new, 0.0);
