...
data {
  ...
  array[N, T] int<lower=-1, upper=1> u; // inputs
  array[N, T] int<lower=-1, upper=1> y; // responses (choices that subject made)
...
transformed data {
  ...
  array[L] real<lower=0> sigma_base;
  ...
  array[L - 2] int<lower=0, upper=L - 2> fixed_kappa_idx = rep_array(0,
                                                                    L - 2);
  ...
  array[L - 1] int<lower=0, upper=L - 1> fixed_omega_idx = rep_array(0,
                                                                    L - 1);
...
transformed parameters {
  ...
  if (n_free_zeta == 1) {
    ...
    zeta_sigma_pr = segment(sigma, 1 + n_free_kappa + n_free_omega,
                            n_free_zeta);
  }
  ...
  if (n_free_kappa > 0) {
    for (i in 1 : n_free_kappa) {
      ...
      vector[N] logit_kappa = mu_kappa_pr[i]
                              + kappa_sigma_pr[i]
                                * segment(kappa_pr, 1 + (i - 1) * N, N);
      kappa[ : , l] = inv_logit_vector_with_bounds(logit_kappa,
                                                   kappa_lower[l],
                                                   kappa_upper[l]);
    }
  }
  ...
  if (n_free_omega > 0) {
    for (i in 1 : n_free_omega) {
      ...
      vector[N] logit_omega = mu_omega_pr[i]
                              + omega_sigma_pr[i]
                                * segment(omega_pr, 1 + (i - 1) * N, N);
      omega[ : , l] = inv_logit_vector_with_bounds(logit_omega,
                                                   omega_lower[l],
                                                   omega_upper[l]);
    }
  }
  ...
  if (n_free_zeta == 1) {
    vector[N] logit_zeta = mu_zeta_pr[1]
                           + zeta_sigma_pr[1] * segment(zeta_pr, 1, N);
...
model {
  ...
  for (i in 1 : N) {
    ...
    array[L] real mu_hat = rep_array(0.0, L); // prior prediction (1 ~ L)
    array[L] real sa_hat = rep_array(0.0, L); // prior uncertainty of prediction (1 ~ L)
...
generated quantities {
  ...
  for (i in 1 : n_free_kappa) {
    int l = free_kappa_idx[i];
    mu_kappa[l] = inv_logit_with_bounds(mu_kappa_pr[i], kappa_lower[l],
                                        kappa_upper[l]);
  }
  for (i in 1 : n_fixed_kappa) {
    int l = fixed_kappa_idx[i];
  ...
  for (i in 1 : n_free_omega) {
    int l = free_omega_idx[i];
    mu_omega[l] = inv_logit_with_bounds(mu_omega_pr[i], omega_lower[l],
                                        omega_upper[l]);
  }
  for (i in 1 : n_fixed_omega) {
    int l = fixed_omega_idx[i];
