...
model {
  ...
  for (i in 1 : N) {
    ...
    for (t in 1 : Tsubj[i]) {
      // compute v_mb
      v_mb[1] = trans_prob * fmax(v_mf[3], v_mf[4])
                + (1 - trans_prob) * fmax(v_mf[5], v_mf[6]); // for level1, stim 1
      v_mb[2] = (1 - trans_prob) * fmax(v_mf[3], v_mf[4])
                + trans_prob * fmax(v_mf[5], v_mf[6]); // for level1, stim 2
      ...
      if (t == 1) {
        level1_prob_choice2 = inv_logit(beta1[i]
                                        * (v_hybrid[2] - v_hybrid[1]));
      } else {
        level1_prob_choice2 = inv_logit(beta1[i]
                                        * (v_hybrid[2] - v_hybrid[1])
                                        + pi[i]
                                          * (2 * level1_choice[i, t - 1] - 3));
      }
      ...
      v_mf[level1_choice[i, t]] += a1[i]
                                   * (v_mf[2 + level2_choice[i, t]]
                                      - v_mf[level1_choice[i, t]]);
      ...
      v_mf[2 + level2_choice[i, t]] += a2[i]
                                       * (reward[i, t]
                                          - v_mf[2 + level2_choice[i, t]]);
      
      // Update Level 1 v_mf
      v_mf[level1_choice[i, t]] += lambda[i] * a1[i]
                                   * (reward[i, t]
                                      - v_mf[2 + level2_choice[i, t]]);
    } // end of t loop
  } // end of i loop
