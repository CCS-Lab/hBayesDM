...
parameters {
  ...
  vector[N] eta_c_pr;  // learning rate
  vector[N] eta_nc_pr; // learning rate
...
transformed parameters {
  ...
  vector<lower=0, upper=1>[N] eta_nc;
  vector[N] alpha;
  vector<lower=0, upper=10>[N] beta;

  for (i in 1:N) {
    eta_c[i]   = Phi_approx(mu_pr[1] + sigma[1] * eta_c_pr[i]);
    eta_nc[i]   = Phi_approx(mu_pr[2] + sigma[2] * eta_nc_pr[i]);
  ...
  for (i in 1:N) {
    for (bIdx in 1:Bsubj[i]) {  // new
      ...
      for (t in 1:(Tsubj[i, bIdx])) {  // new
        ...
        ev[choice[i, bIdx, t]]   += eta_c[i] * PE;   //new
        ev[3-choice[i, bIdx, t]] += eta_nc[i] * PEnc;  //new
      } // end of t loop
    } // end of bIdx loop
  } // end of i loop
}
...
generated quantities {
  ...
  real<lower=0, upper=1> mu_eta_c;
  real<lower=0, upper=1> mu_eta_nc;
  real mu_alpha;
  ...
  mu_eta_c  = Phi_approx(mu_pr[1]);
  mu_eta_nc = Phi_approx(mu_pr[2]);
  mu_alpha  = mu_pr[3];
