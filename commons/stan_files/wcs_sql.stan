...
data {
  ...
  array[T] matrix<lower=0, upper=1>[3, 4] deck_match_rule; // indicates which dimension(color, form, number) each of the 4 decks matches to within a trial
}
transformed data {
...
model {
  ...
  for (i in 1 : N) {
    ...
    for (t in 1 : Tsubj[i]) {
      ...
      if (t < Tsubj[i]) {
        tmpp = to_vector(tmpatt * deck_match_rule[t + 1,  : ,  : ]) * .9998
               + .0001;
