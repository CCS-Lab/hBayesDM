...
data {
  ...
  array[N, T] int<lower=-1> Isubj; // Trial number for each task condition
...
transformed parameters {
  ...
  for (i in 1 : N) {
    ...
    tau[i] = Phi_approx(mu_pr[3] + sigma[3] * tau_pr[i])
...
model {
  ...
  for (i in 1 : N) {
    ...
    for (t in 1 : Tsubj[i]) {
      ...
      if (r == 1) {
        RT[i, t] ~ wiener(a[i] * (Isubj[i, t] / 10.0) ^ bp[i], tau[i], 0.5,
                          d);
      } else {
        RT[i, t] ~ wiener(a[i] * (Isubj[i, t] / 10.0) ^ bp[i], tau[i], 0.5,
                          -d);
  ...
  
  {
    ...
    for (i in 1 : N) {
      ...
      for (t in 1 : Tsubj[i]) {
        ...
        // Drift diffusion process
        if (r == 1) {
          log_lik[i] += wiener_lpdf(RT[i, t] | a[i]
                                               * (Isubj[i, t] / 10.0) ^ bp[i], tau[i], 0.5, d);
        } else {
          log_lik[i] += wiener_lpdf(RT[i, t] | a[i]
                                               * (Isubj[i, t] / 10.0) ^ bp[i], tau[i], 0.5, -d);
        }
        ...
        // Drift diffusion process
        tmp_sm = wiener_rng(a[i] * (Isubj[i, t] / 10.0) ^ bp[i], tau[i], 0.5,
                            d_sm);
