...
model {
  ...
  for (i in 1 : N) {
    for (t in 1 : Tsubj[i]) {
      ...
      if (opt1hval[i, t] > 0) {
        if (opt1lval[i, t] >= 0) {
          U_opt[1] = w_prob[1] * (opt1hval[i, t] ^ rho[i])
                     + w_prob[2] * (opt1lval[i, t] ^ rho[i]);
        } else {
          U_opt[1] = w_prob[1] * (opt1hval[i, t] ^ rho[i])
                     - w_prob[2] * (abs(opt1lval[i, t]) ^ rho[i])
                       * lambda[i];
        }
      } else {
      ...
      if (opt2hval[i, t] > 0) {
        if (opt2lval[i, t] >= 0) {
          U_opt[2] = w_prob[3] * (opt2hval[i, t] ^ rho[i])
                     + w_prob[4] * (opt2lval[i, t] ^ rho[i]);
        } else {
          U_opt[2] = w_prob[3] * (opt2hval[i, t] ^ rho[i])
                     - w_prob[4] * (abs(opt2lval[i, t]) ^ rho[i])
                       * lambda[i];
        }
      } else {
