  ...
  for (i in 1 : N) {
    ...
    for (t in 1 : Tsubj[i]) {
      ...
      if (pressed[i, t]) {
        ...
        if (outcome[i, t] >= 0) {
          qv_g[cue[i, t]] += ep[i]
                             * (rhoRew[i] * outcome[i, t] - qv_g[cue[i, t]]);
        } else {
          qv_g[cue[i, t]] += ep[i]
                             * (rhoPun[i] * outcome[i, t] - qv_g[cue[i, t]]);
        }
      } else {
        // update no-go value
        if (outcome[i, t] >= 0) {
          qv_ng[cue[i, t]] += ep[i]
                              * (rhoRew[i] * outcome[i, t] - qv_ng[cue[i, t]]);
        } else {
          qv_ng[cue[i, t]] += ep[i]
                              * (rhoPun[i] * outcome[i, t] - qv_ng[cue[i, t]]);
        }
      }
    } // end of t loop
  } // end of i loop
  ...
  
  {
    ...
    for (i in 1 : N) {
      ...
      for (t in 1 : Tsubj[i]) {
        ...
        if (outcome[i, t] >= 0) {
          sv[cue[i, t]] += ep[i]
                           * (rhoRew[i] * outcome[i, t] - sv[cue[i, t]]);
        } else {
          sv[cue[i, t]] += ep[i]
                           * (rhoPun[i] * outcome[i, t] - sv[cue[i, t]]);
        }
        ...
        if (pressed[i, t]) {
          ...
          if (outcome[i, t] >= 0) {
            qv_ng[cue[i, t]] += ep[i]
                                * (rhoRew[i] * outcome[i, t]
                                   - qv_ng[cue[i, t]]);
          } else {
            qv_ng[cue[i, t]] += ep[i]
                                * (rhoPun[i] * outcome[i, t]
                                   - qv_ng[cue[i, t]]);
          }
        }
      } // end of t loop
