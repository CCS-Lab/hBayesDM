...
model {
  ...
  for (i in 1:N) {
    ...
    vector[3] tmpatt;       // temporary variable to calculate att
    ...
    for (t in 1:Tsubj[i]) {
      tmpatt[1] = pow(att[1], d[i]);
      tmpatt[2] = pow(att[2], d[i]);
      tmpatt[3] = pow(att[3], d[i]);

      // Calculate p_choice in the given trial
      p_choice = to_vector(tmpatt' * match[t]);
      p_choice /= sum(p_choice);
      // p_choice = p_choice * (1 - EPS) + EPS;
      // p_choice /= sum(p_choice);
