...
model {
  ...
  for (i in 1:N) {
    ...
    level1_prob_choice2 = 0.5; 
    level2_prob_choice2 = 0.5; 
    ...
    for (t in 2:Tsubj[i])  {
      ...
      // Prob of choosing stim 2 (1 from [1,2] OR 3 from [3,4]) in ** Level (step) 2 **
      ...
      // Level 1--> choose one of two level 1 options
      ...
      // Update Level 1 v_mf
      v_mf[level1_choice[i,t]] = v_mf[level1_choice[i,t]] + a1[i]*(v_mf[2+ level2_choice[i,t]] - v_mf[ level1_choice[i,t]]) + lambda[i] * a1[i] * (reward[i,t] - v_mf[level1_choice[i,t]]);

      // compute v_mb
      v_mb[1] = 0.7 * fmax(v_mf[3], v_mf[4]) + 0.3 * fmax(v_mf[5], v_mf[6]); // for level1, stim 1
      v_mb[2] = 0.3 * fmax(v_mf[3], v_mf[4]) + 0.7 * fmax(v_mf[5], v_mf[6]); // for level1, stim 2
...
generated quantities {
  ...
  for (i in 1:N) {
    ...
    for (t in 2:Tsubj[i])  {
      ...
      // Update Level 1 v_mf
      v_mf[level1_choice[i,t]] = v_mf[level1_choice[i,t]] + a1[i]*(v_mf[2+ level2_choice[i,t]] - v_mf[ level1_choice[i,t]]) + lambda[i] * a1[i] * (reward[i,t] - v_mf[2+level2_choice[i,t]]);

      // compute v_mb
      v_mb[1] = 0.7 * fmax(v_mf[3], v_mf[4]) + 0.3 * fmax(v_mf[5], v_mf[6]); // for stim 1
      v_mb[2] = 0.3 * fmax(v_mf[3], v_mf[4]) + 0.7 * fmax(v_mf[5], v_mf[6]); // for stim 2
