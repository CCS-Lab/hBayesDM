...
In constrast, ω₃ showed almost no recovery with values more scattered compared to TAPAS,
...
with positive correlations almost near 1.
...
Since the `vb` option has the issue of failing stochastically, consider using the `random` option as below
...
The main issue with the data used in [Section 4](#parameter-recovery) was that the individual difference in ω₃ 
...
## 5.3. Estimating parameters at high level

Many research has reported issues in estimating parameters at level 3 or above such as ω₃ [@reed2020paranoia; @hein2021state; @tecilla2023modulation].
