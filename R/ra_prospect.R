...
ra_prospect <- model_base(
  ...
  preprocess_function = function(rawdata) {
    ...
    for (i in 1:n_subj)  {
      subj <- subjs[i]
      t_subjs[i] <- sum(rawdata$subjid == subj)
    }
    ...
    for (i in 1:n_subj) {
      ...
      gain[i, 1:t]   <- tmp[1:t, "gain"]
      loss[i, 1:t]   <- abs(tmp[1:t, "loss"])  # absolute loss amount
      cert[i, 1:t]   <- tmp[1:t, "cert"]
      gamble[i, 1:t] <- tmp[1:t, "gamble"]
    }
